    ...
    success: (res) => {
      ...
      wx.login({
        success: (loginRes) => {
          ...
          // 发送请求到后台服务器
          wx.request({
            url: app.globalData.baseUrl + '/iot/miniApp/wxLogin',
            method: 'POST',
            data: {
              appid: 'wx62c19a65d8733102',
              code:code,
            ...
            success: (requestRes) => {
              ...
              wx.setStorageSync('wxLogin', requestRes.data.data)
              let userId = requestRes.data.data.userId;
              this.setData({
                userId:userId
              })
              wx.navigateTo({
                url: '/pages/userlogininfo/userlogininfo?userId=' + this.data.userId,
              })
            },
            fail: (error) => {
