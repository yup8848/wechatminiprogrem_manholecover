...
Page({
  ...
  mySelect:function(e){
    ...
    console.log('mySelect:',e.target.dataset.me)
    if (e.target.dataset.me=='正常') {
      console.log('当前点击选择是正常')
      this.setData({
        siteState:20
      })
      this.loadAndDisplayMarkers()
    }
    if (e.target.dataset.me=='异常') {
      console.log('当前点击选择是异常')
      this.setData({
        siteState:30
      })
      this.loadAndDisplayMarkers()
       ...
       // 定义请求地址
   const apiUrl = app.globalData.baseUrl + '/iot/welllid/siteState'; 
   // 发起GET请求
   wx.request({
     url: apiUrl,
     method: 'GET',
     ...
     success: (res) =>  {
       // 请求成功的回调函数
       // 在这里处理从服务器获取到的数据
       console.log('站点状态siteState请求成功', res);
       ...
       if (storedPositions && storedPositions.length > 0) {
         const allMarkers = storedPositions.map((position, index) => {
           return {
             id: index,
             ...
             width: 24,  
             height: 30,
             positionName: position.siteName ,
           };
         });
   
          // 计算地图中心点，可调整
          const centerLatitude = storedPositions.reduce((sum, position) => sum + position.latitude, 0) / storedPositions.length;
          const centerLongitude = storedPositions.reduce((sum, position) => sum + position.longitude, 0) / storedPositions.length;
...
//   },
  ...
  loadAndDisplayMarkers: function () {
   // 定义请求地址
   const apiUrl = app.globalData.baseUrl + '/iot/welllid/mapSiteList'; 
   // 发起GET请求
   wx.request({
     url: apiUrl,
     method: 'GET',
  ...
  search: function () {
   ...
   markerTap: function (e) {
     ...
     // 在这里你可以添加点击标记点后的逻辑
      // 更新文本框内容为完整数据集信息
      const storedPositions = wx.getStorageSync('mapPositions');
      const contentStringPosition = JSON.stringify(storedPositions[e.markerId])
      wx.showModal({
        title: '站点的详细数据信息',
